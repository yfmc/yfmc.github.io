...
public class SessionController {
	/**
	 * 세션 저장을 위한 작성 페이지
	 * --> 이페이지를 "/session/write.do" URL에 GET방식으로 노출시킨다.
	 */
	@RequestMapping(value = "/login_ok.do", method = RequestMethod.GET)
	public String sessionWrite(Model model, HttpServletRequest request) {
		/* 컨트롤러에서 세션을 식별하기 위한 처리 */
		/* 세션값을 request 내장객체를 통해서 HttpSession객체를 생성해야 접근할 수 있다. */
		/*-->Servlet과 동일함*/
		...
		//"/src/main/webapp/WEB-INF/views/session/write.jsp" 파일을 View로 지정
		return "session/write";
	}
	
	/**
	 * 세션 저장을 위한 action 페이지
	 * --> 이페이지를 "/session/save.do" URL에 POST방식으로 노출시킨다.
	 */
	...
	public String sessionSave(HttpServletRequest request,
		...
		// '/'부터 시작할 경우 ContextPath는 자동으로 앞에 추가도니다.
