...
var b_json={
	 ...
	 "충청세종": [
		 {"brano": 6001, "brand": "메가박스", "branch": "공주"},
		  ...
		  {"brano": 6028, "brand": "CGV", "branch": "충성,세종논산"},
	 ...
	 "부산울산": [
		 {"brano": 10001, "brand": "롯데시네마", "branch": "광복"},
		  ...
		  {"brano": 10008, "brand": "롯데시네마", "branch": "동부산,울산아울렛"},
		  {"brano": 10009, "brand": "메가박스", "branch": "부산,울산극장"},
		  {"brano": 10010, "brand": "메가박스", "branch": "부산,울산대"},
		  {"brano": 10011, "brand": "롯데시네마", "branch": "부산,울산본점"},
		  ...
		  {"brano": 10019, "brand": "롯데시네마", "branch": "오투(부산,울산대)"},
...
$(function() {
	...
	$("#sel_region").change(function() {
		...
		var html = "";
		for (var i=0; i<b_json.서울.length; i++) {
			html += "<option value=" + b_json.서울[i].brand + "&nbsp"+ b_json.서울[i].branch + ">" + b_json.서울[i].brand + " " + b_json.서울[i].branch + "</option>\n";
		}
			$("#sel_theater").html(html);
		} else if (region == "경기") {
			// 경기 지역 영화관 드롭다운
			var html = "";
			for (var i=0; i<b_json.경기.length; i++) {
				html += "<option value=" + b_json.경기[i].brand + "&nbsp"+ b_json.경기[i].branch + ">" + b_json.경기[i].brand + " " + b_json.경기[i].branch + "</option>\n";
			}
				$("#sel_theater").html(html);
		} else if (region == "인천") {
			// 인천 지역 영화관 드롭다운
			var html = "";
			for (var i=0; i<b_json.인천.length; i++) {
				html += "<option value=" + b_json.인천[i].brand + "&nbsp"+ b_json.인천[i].branch + ">" + b_json.인천[i].brand + " " + b_json.인천[i].branch + "</option>\n";
			}
				$("#sel_theater").html(html);
		} else if (region == "강원") {
			// 강원 지역 영화관 드롭다운
			var html = "";
			for (var i=0; i<b_json.강원.length; i++) {
				html += "<option value=" + b_json.강원[i].brand + "&nbsp"+ b_json.강원[i].branch + ">" + b_json.강원[i].brand + " " + b_json.강원[i].branch + "</option>\n";
			}
				$("#sel_theater").html(html);
		} else if (region == "대전") {
			// 대전 지역 영화관 드롭다운
			var html = "";
			for (var i=0; i<b_json.대전.length; i++) {
				html += "<option value=" + b_json.대전[i].brand + "&nbsp"+ b_json.대전[i].branch + ">" + b_json.대전[i].brand + " " + b_json.대전[i].branch + "</option>\n";
			}
				$("#sel_theater").html(html);
		} else if (region == "충청세종") {
			// 충성,세종/세종 지역 영화관 드롭다운
			var html = "";
			for (var i=0; i<b_json.충청세종.length; i++) {
				html += "<option value=" + b_json.충청세종[i].brand + "&nbsp"+ b_json.충청세종[i].branch + ">" + b_json.충청세종[i].brand + " " + b_json.충청세종[i].branch + "</option>\n";
			}
				$("#sel_theater").html(html);
		} else if (region == "광주") {
			// 광주 지역 영화관 드롭다운
			var html = "";
			for (var i=0; i<b_json.광주.length; i++) {
				html += "<option value=" + b_json.광주[i].brand + "&nbsp"+ b_json.광주[i].branch + ">" + b_json.광주[i].brand + " " + b_json.광주[i].branch + "</option>\n";
			}
				$("#sel_theater").html(html);
		} else if (region == "전라") {
			// 전라 지역 영화관 드롭다운
			var html = "";
			for (var i=0; i<b_json.전라.length; i++) {
				html += "<option value=" + b_json.전라[i].brand + "&nbsp"+ b_json.전라[i].branch + ">" + b_json.전라[i].brand + " " + b_json.전라[i].branch + "</option>\n";
			}
				$("#sel_theater").html(html);
		} else if (region == "대구경북") {
			// 대구,경북 지역 영화관 드롭다운
			var html = "";
			for (var i=0; i<b_json.대구경북.length; i++) {
				html += "<option value=" + b_json.대구경북[i].brand + "&nbsp"+ b_json.대구경북[i].branch + ">" + b_json.대구경북[i].brand + " " + b_json.대구경북[i].branch + "</option>\n";
			}
				$("#sel_theater").html(html);
		} else if (region == "부산울산") {
			// 부산,울산/울산 지역 영화관 드롭다운
			var html = "";
			for (var i=0; i<b_json.부산울산.length; i++) {
				html += "<option value=" + b_json.부산울산[i].brand + "&nbsp"+ b_json.부산울산[i].branch + ">" + b_json.부산울산[i].brand + " " + b_json.부산울산[i].branch + "</option>\n";
			}
				$("#sel_theater").html(html);
		} else if (region == "경남") {
			// 경남 지역 영화관 드롭다운
			var html = "";
			for (var i=0; i<b_json.경남.length; i++) {
				html += "<option value=" + b_json.경남[i].brand + "&nbsp"+ b_json.경남[i].branch + ">" + b_json.경남[i].brand + " " + b_json.경남[i].branch + "</option>\n";
			}
				$("#sel_theater").html(html);
		} else if (region == "제주") {
			// 제주 지역 영화관 드롭다운 
			var html = "";
			for (var i=0; i<b_json.제주.length; i++) {
				html += "<option value=" + b_json.제주[i].brand + "&nbsp"+ b_json.제주[i].branch + ">" + b_json.제주[i].brand + " " + b_json.제주[i].branch + "</option>\n";
			}
				$("#sel_theater").html(html);
		}
	});
